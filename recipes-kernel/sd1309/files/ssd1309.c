...
struct i2c_board_info ssd1309_board_info = {
        .type = "ssd1309",
        .addr = I2C_ADDR,
};
...
static int ssd1309_driver_init(void) {
    int ret;
    struct i2c_adapter *my_i2c_adapter;

    // Tìm I2C adapter theo tên "i2c-1"	
	printk("[PAD] %s - %d\n", __FUNCTION__, __LINE__);
	...
	printk("[PAD] %s - %d\n", __FUNCTION__, __LINE__);
    // Đặt thông tin thiết bị I2C
    my_i2c_client = i2c_new_client_device(my_i2c_adapter, &ssd1309_board_info);
	...
	printk("[PAD] %s - %d\n", __FUNCTION__, __LINE__);
    ret = i2c_add_driver(&ssd1307fb_driver);
    if (ret) {
...
static void __exit ssd1309_driver_exit(void) {
    i2c_unregister_device(my_i2c_client);
    i2c_del_driver(&ssd1307fb_driver);
}

module_init(ssd1309_driver_init);
module_exit(ssd1309_driver_exit);

MODULE_DESCRIPTION("FB driver for the Solomon SSD1307 OLED controller");
MODULE_AUTHOR("pad <phamanhduc11@gmail.com>");
MODULE_LICENSE("GPL");
