...
/*
 * Driver for the Solomon SSD1309 OLED controller
 *
...
#define SSD1309FB_DATA			0x40
#define SSD1309FB_COMMAND		0x80
...
#define SSD1309FB_SET_LOOKUP_TABLE	0x91
...
#define	SSD1309FB_SET_AREA_COLOR_MODE	0xd8
...
static int SSD1309fb_write_array(struct i2c_client *client,
...
static inline int SSD1309fb_write_cmd(struct i2c_client *client, u8 cmd)
{
	struct SSD1309fb_array *array;
	int ret;
	...
	ret = SSD1309fb_write_array(client, array, 1);
	kfree(array);

	return ret;
}

static int SSD1309fb_set_col_range(struct SSD1309fb_par *par, u8 col_start,
				   u8 cols)
{
	...
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SET_COL_RANGE);
	if (ret < 0)
		return ret;

	ret = SSD1309fb_write_cmd(par->client, col_start);
	if (ret < 0)
		return ret;

	ret = SSD1309fb_write_cmd(par->client, col_end);
	if (ret < 0)
		return ret;
...
static int SSD1309fb_set_page_range(struct SSD1309fb_par *par, u8 page_start,
				    u8 pages)
{
	...
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SET_PAGE_RANGE);
	if (ret < 0)
		return ret;

	ret = SSD1309fb_write_cmd(par->client, page_start);
	if (ret < 0)
		return ret;

	ret = SSD1309fb_write_cmd(par->client, page_end);
	if (ret < 0)
		return ret;
...
static int SSD1309fb_update_rect(struct SSD1309fb_par *par, unsigned int x,
				 ...
				 unsigned int height)
{
	...
	array = SSD1309fb_alloc_array(width * pages, SSD1309FB_DATA);
	if (!array)
		return -ENOMEM;
	...
	ret = SSD1309fb_set_col_range(par, par->col_offset + x, width);
	if (ret < 0)
		goto out_free;

	ret = SSD1309fb_set_page_range(par, par->page_offset + y / 8, pages);
	if (ret < 0)
		goto out_free;
	...
	ret = SSD1309fb_write_array(par->client, array, width * pages);
...
static int SSD1309fb_update_display(struct SSD1309fb_par *par)
{
	return SSD1309fb_update_rect(par, 0, 0, par->width, par->height);
}
		...
		size_t count, loff_t *ppos)
{
	...
	ret = SSD1309fb_update_display(par);
	if (ret < 0)
		return ret;
...
static int SSD1309fb_blank(int blank_mode, struct fb_info *info)
{
	...
	if (blank_mode != FB_BLANK_UNBLANK)
		return SSD1309fb_write_cmd(par->client, SSD1309FB_DISPLAY_OFF);
	else
		return SSD1309fb_write_cmd(par->client, SSD1309FB_DISPLAY_ON);
}
...
static void SSD1309fb_fillrect(struct fb_info *info, const struct fb_fillrect *rect)
{
	...
	SSD1309fb_update_rect(par, rect->dx, rect->dy, rect->width,
			      rect->height);
}
...
static void SSD1309fb_copyarea(struct fb_info *info, const struct fb_copyarea *area)
{
	...
	SSD1309fb_update_rect(par, area->dx, area->dy, area->width,
			      area->height);
}
...
static void SSD1309fb_imageblit(struct fb_info *info, const struct fb_image *image)
{
	...
	SSD1309fb_update_rect(par, image->dx, image->dy, image->width,
			      image->height);
}

static const struct fb_ops SSD1309fb_ops = {
	.owner		= THIS_MODULE,
	.fb_read	= fb_sys_read,
...
static int SSD1309fb_init(struct SSD1309fb_par *par)
{
	...
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_CONTRAST);
	if (ret < 0)
		return ret;

	ret = SSD1309fb_write_cmd(par->client, par->contrast);
	if (ret < 0)
		return ret;

	/* Set segment re-map */
	if (par->seg_remap) {
		ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SEG_REMAP_ON);
		if (ret < 0)
			return ret;
	}
	...
	ret = SSD1309fb_write_cmd(par->client,  com_invdir);
	if (ret < 0)
		return ret;

	/* Set multiplex ratio value */
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SET_MULTIPLEX_RATIO);
	if (ret < 0)
		return ret;

	ret = SSD1309fb_write_cmd(par->client, par->height - 1);
	if (ret < 0)
		return ret;

	/* set display offset value */
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SET_DISPLAY_OFFSET);
	if (ret < 0)
		return ret;

	ret = SSD1309fb_write_cmd(par->client, par->com_offset);
	if (ret < 0)
		return ret;

	/* Set clock frequency */
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SET_CLOCK_FREQ);
	if (ret < 0)
		return ret;

	dclk = ((par->dclk_div - 1) & 0xf) | (par->dclk_frq & 0xf) << 4;
	ret = SSD1309fb_write_cmd(par->client, dclk);
	if (ret < 0)
		return ret;
	...
	if (par->area_color_enable || par->low_power) {
		...
		ret = SSD1309fb_write_cmd(par->client,
					  SSD1309FB_SET_AREA_COLOR_MODE);
		if (ret < 0)
			return ret;

		mode = (par->area_color_enable ? 0x30 : 0) |
			(par->low_power ? 5 : 0);
		ret = SSD1309fb_write_cmd(par->client, mode);
		if (ret < 0)
			return ret;
	}

	/* Set precharge period in number of ticks from the internal clock */
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SET_PRECHARGE_PERIOD);
	if (ret < 0)
		return ret;

	precharge = (par->prechargep1 & 0xf) | (par->prechargep2 & 0xf) << 4;
	ret = SSD1309fb_write_cmd(par->client, precharge);
	if (ret < 0)
		return ret;

	/* Set COM pins configuration */
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SET_COM_PINS_CONFIG);
	if (ret < 0)
		return ret;

	compins = 0x02 | !par->com_seq << 4 | par->com_lrremap << 5;
	ret = SSD1309fb_write_cmd(par->client, compins);
	if (ret < 0)
		return ret;

	/* Set VCOMH */
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SET_VCOMH);
	if (ret < 0)
		return ret;

	ret = SSD1309fb_write_cmd(par->client, par->vcomh);
	if (ret < 0)
		return ret;

	/* Turn on the DC-DC Charge Pump */
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_CHARGE_PUMP);
	if (ret < 0)
		return ret;
		...
		for (i = 0; i < ARRAY_SIZE(par->lookup_table); ++i) {
			...
			if (val < 31 || val > 63)
				dev_warn(&par->client->dev,
					 ...
					 i, val);
			ret = SSD1309fb_write_cmd(par->client, val);
			if (ret < 0)
				return ret;
		}
	}
	...
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_SET_ADDRESS_MODE);
	if (ret < 0)
		return ret;
	...
	/* Turn on the display */
	ret = SSD1309fb_write_cmd(par->client, SSD1309FB_DISPLAY_ON);
	if (ret < 0)
		return ret;
...
static struct SSD1309fb_deviceinfo SSD1309fb_ssd1305_deviceinfo = {
	.default_vcomh = 0x34,
	.default_dclk_div = 1,
	.default_dclk_frq = 7,
};

static struct SSD1309fb_deviceinfo SSD1309fb_ssd1306_deviceinfo = {
	.default_vcomh = 0x20,
	.default_dclk_div = 1,
	.default_dclk_frq = 8,
	.need_chargepump = 1,
...
static const struct of_device_id SSD1309fb_of_match[] = {
	{
		.compatible = "solomon,ssd1305fb-i2c",
		.data = (void *)&SSD1309fb_ssd1305_deviceinfo,
	},
	{
		.compatible = "solomon,ssd1306fb-i2c",
		.data = (void *)&SSD1309fb_ssd1306_deviceinfo,
	},
	{
...
MODULE_DEVICE_TABLE(of, SSD1309fb_of_match);

static int SSD1309fb_probe(struct i2c_client *client)
{
	...
	info = framebuffer_alloc(sizeof(struct SSD1309fb_par), dev);
	if (!info)
		return -ENOMEM;
	...
	SSD1309fb_defio = devm_kzalloc(dev, sizeof(*SSD1309fb_defio),
				       GFP_KERNEL);
	if (!SSD1309fb_defio) {
	...
	bl = backlight_device_register(bl_name, dev, par, &SSD1309fb_bl_ops,
				       NULL);
	if (IS_ERR(bl)) {
		ret = PTR_ERR(bl);
...
static int SSD1309fb_remove(struct i2c_client *client)
{
	struct fb_info *info = i2c_get_clientdata(client);
	struct SSD1309fb_par *par = info->par;

	SSD1309fb_write_cmd(par->client, SSD1309FB_DISPLAY_OFF);
...
static const struct i2c_device_id SSD1309fb_i2c_id[] = {
	{ "ssd1305fb", 0 },
	{ "ssd1306fb", 0 },
	{ "SSD1309fb", 0 },
	{ "ssd1309fb", 0 },
...
MODULE_DEVICE_TABLE(i2c, SSD1309fb_i2c_id);

static struct i2c_driver SSD1309fb_driver = {
	.probe_new = SSD1309fb_probe,
	.remove = SSD1309fb_remove,
	.id_table = SSD1309fb_i2c_id,
	.driver = {
		.name = "SSD1309fb",
		.of_match_table = SSD1309fb_of_match,
	},
};

module_i2c_driver(SSD1309fb_driver);

MODULE_DESCRIPTION("FB driver for the Solomon SSD1309 OLED controller");
